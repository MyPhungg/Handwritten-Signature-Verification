...
class KhachHang:
    def __init__(self, CCCD: str, HoTen: str, DiaChi: str,  ChuKy: str, Email: str, NgaySinh: str, NgayCapCCCD: str, NgayHetHanCCCD: str, MaNVQL: str):
        ...
        try:
            ...
            today = date.today()
            if NgaySinh > today:
                raise ValueError("Lỗi: Ngày cấp CCCD không được vượt quá hiện tại.")
            self.NgayCapCCCD = ngaycap
        except ValueError:
            raise ValueError("Ngày cấp CCCD không hợp lệ! Vui lòng nhập theo định dạng YYYY-MM-DD.")
        ...
        try:
            ...
            today = date.today()
            if NgaySinh > today:
                raise ValueError("Lỗi: Ngày sinh không được vượt quá hiện tại.")
            self.NgaySinh = ngay_sinh
        except ValueError:
            raise ValueError("Ngày sinh không hop lệ! Vui lòng nhập theo định dạng YYYY-MM-DD.")
...
class TaiKhoan:
    def __init__(self, MaTK: str, LoaiTK: str, SoDu: str):
        self.MaTK = MaTK
        self.LoaiTK = LoaiTK
        self.SoDu = Decimal(SoDu) #Decimal quan ly chinh xac so tien hon float
